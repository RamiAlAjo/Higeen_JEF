...
<head>
    <meta charset="UTF-8">
    <title>{{ __('Top Sales by Area') }}</title>
    <style>
        body {
...
<body>
    ...
    @if($startDate && $endDate)
        <p class="date-range">
            {{ __('Date Range: :start to :end', ['start' => $startDate, 'end' => $endDate]) }}
        </p>
    @endif
        ...
        <table>
            ...
            <tbody>
                @foreach($topSalesByArea as $area)
                    <tr>
                        <td>{{ $area->area_name ?? __('Unknown') }}</td>
                        <td>{{ number_format($area->total_sales ?? 0, 2) }} {{ __('JOD') }}</td>
                        <td>{{ $area->order_count ?? 0 }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
